 ...
 import { computed, onMounted, reactive, ref,watch,nextTick,toRefs, watchEffect,getCurrentInstance} from 'vue';
    ...
    export default {
        ...
        props: {
            ...
            explain: {
                type: String,
                default: ()=>useI18n().t('extra.slideToRight')
            },
            imgSize: {
                type: Object,
                default() {
        ...
        setup(props,context){
          const {t}=useI18n()
            const {mode,captchaType,vSpace,imgSize,barSize,type,blockSize,explain,request} = toRefs(props)
            ...
            let secretKey = ref(''),           //后端返回的ase加密秘钥
              ...
              async function end() {
                    ...
                    if (status.value && isEnd.value == false) {
                     ...
                     const code=await props.request(data)
                      ...
                      // reqCheck(data).then(res=>{
                            if (code == "0") {
                                ...
                                passFlag.value = true
                                tipWords.value = `${((endMovetime.value-startMoveTime.value)/1000).toFixed(2)}s${t('extra.verSuccess')}`
                                ...
                                tipWords.value = t('extra.verFail')
                              if (mode.value=='pop') {
                                // setTimeout(()=>{
                                proxy.$parent.clickShow = false;
                                // refresh();
