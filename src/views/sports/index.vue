    ...
    <d2-container class="mt-30">
      <ul class="flex-box-between">
        <li class="game-item flex-box is-center"
            :style="{backgroundImage:`url(${item.rlicon ? item.rlicon : item.id ? sportbg:coming})`}"
            @click="startGame(item)"
            v-for="item in  gameList"
        >
          <div class="game-info is-bold" :class="{'has-game':item.icon}">
            <img v-if="item.img" :src="item.img" alt="" class="timg" @load="hdonload(item)">
...
function hdonload(item) {
  item.rlicon = item.img
}

const getGameList = () => {
  const params={ current: 1, size: 18, data: { category: 1, ids: [101] }, sign: "" }
  $post("slotGameList", params).then((res) => {
    if (res.code === 0) {
      state.platList = res.data.list;
    }
  })
};
...
const startGame = async ({ id, platId=1, gameId }) => {
  if(id){
    if (getToken()) {
      $post("startGame", { id, gameId: gameId, playMode: 1 }).then((res) => {
        if (res.code === 0) {
          router.push({
            name: "gameRoom",
...
.cockfight-view{
  ...
  .game-item{
    ...
    background: url(@/assets/images/game/coming-soon.png) no-repeat center center;
